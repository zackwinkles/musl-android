...
static void mi_vfprintf_thread(mi_output_fun* out, void* arg, const char* prefix, const char* fmt, va_list args) {
  if (prefix != NULL && strnlen(prefix,33) <= 32 && !_mi_is_main_thread()) {
    char tprefix[64];
...
static void mi_option_init(mi_option_desc_t* desc) {
  ...
  char s[64 + 1];
  char buf[64+1];
  strlcpy(buf, "mimalloc_", sizeof(buf));
  strlcat(buf, desc->name, sizeof(buf));
  bool found = _mi_prim_getenv(buf, s, sizeof(s));
  if (!found && desc->legacy_name != NULL) {
    strlcpy(buf, "mimalloc_", sizeof(buf));
    strlcat(buf, desc->legacy_name, sizeof(buf));
    found = _mi_prim_getenv(buf, s, sizeof(s));
    if (found) {
