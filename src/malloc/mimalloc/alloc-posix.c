...
mi_decl_restrict void* mi_memalign(size_t alignment, size_t size) {
  void* p = mi_malloc_aligned(size, alignment);
  mi_assert_internal(((uintptr_t)p % alignment) == 0);
  return p;
}

mi_decl_restrict void* mi_valloc(size_t size) {
  return mi_memalign( _mi_os_page_size(), size );
}

mi_decl_restrict void* mi_pvalloc(size_t size) {
...
mi_decl_restrict void* mi_aligned_alloc(size_t alignment, size_t size) {
...
void* mi__expand(void* p, size_t newsize) {  // Microsoft
  void* res = mi_expand(p, newsize);
  if (res == NULL) { errno = ENOMEM; }
  return res;
}

mi_decl_restrict unsigned short* mi_wcsdup(const unsigned short* s) {
...
mi_decl_restrict unsigned char* mi_mbsdup(const unsigned char* s)  {
...
void* mi_aligned_offset_recalloc(void* p, size_t newcount, size_t size, size_t alignment, size_t offset) { // Microsoft
  return mi_recalloc_aligned_at(p, newcount, size, alignment, offset);
}

void* mi_aligned_recalloc(void* p, size_t newcount, size_t size, size_t alignment) { // Microsoft
